...
Defines the taxable basis on which composite products (bundles, configurables, grouped) are reported to TaxCloud, so that lookup, capture, refund and the native tax fallback all describe one order by the same lines at the same quantities.
