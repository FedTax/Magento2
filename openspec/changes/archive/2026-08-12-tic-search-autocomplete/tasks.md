...
## 12. CI fixes (found after archiving, by the full matrix)

- [x] 12.1 **Unit, Magento 2.4.9 / PHP 8.5.** `ReflectionMethod::setAccessible()` is deprecated in PHP 8.5 and Magento's unit bootstrap escalates deprecations to failures. It has been a no-op since 8.1, so both calls were removed — `Test/Unit/Controller/Adminhtml/Tic/SearchTest.php` and `Test/Integration/Model/UserAgentWiringTest.php` (the latter would have failed the 2.4.9 integration job next)
- [x] 12.2 **Integration, all six jobs.** The three live-lookup tests failed with `transport` on every version. Cause: the install seeds `api_type=soap`, so the router chose the SOAP backend — and this suite mocks SOAP process-wide. `installSoapMock()` replaces the shared `ClientFactory`, that instance outlives the test that installed it, and `RecordingSoapClient` throws for any method with no canned response, `GetTICs` included. The tests were therefore asserting against a mock that always fails: **green in isolation, red in the full suite**, which is exactly how it presented
- [x] 12.3 Live assertions moved onto the REST backend explicitly, bypassing the router and the SOAP mock. Live SOAP TIC lookup keeps its coverage in `Test/E2e/specs/admin/admin-tic-search.spec.ts`, which drives a real store whose `api_type` is `soap` — the one place it can be exercised honestly
- [x] 12.4 Live assertions loosened to what TaxCloud actually guarantees: a plain-English query returns labelled, numerically-coded TICs including at least one relevant match, rather than a specific code at a specific rank. Semantic ranking is TaxCloud's to change, and pinning it would make CI fail on their release schedule, not on our regressions
- [x] 12.5 Verified by running the **full** integration suite locally in the failing order, not the filtered subset that hid the bug: 71 tests, 246 assertions, no deprecations
